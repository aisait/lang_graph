...
## OpenIA API Response 1.50.0 + LangGraph + Langfuse v3.09.0 (SDK Nativo) + OpenTelemetry (Infraestructura)
...
4. **LangGraph → Langfuse**: El `CallbackHandler` de Langfuse captura cada ejecución del grafo (prompts, respuestas, tokens, costos) y los envía a la API HTTP de Langfuse (no mediante OTLP). Las trazas se almacenan en `langfuse-clickhouse` y los metadatos en `langfuse-postgres`.
