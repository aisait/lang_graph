...
def extraer_intencion_humana(messages: list) -> str:
    ...
    Soporta tanto strings como listas de contenido (multimodal/WhatsApp).
    Prueba de caja negra: BC‑T01, BC‑T02.
    """
    for msg in reversed(messages):
        if isinstance(msg, HumanMessage):
            ...
            if isinstance(content, str):
                ...
                # Extraer texto de contenido multimodal (WhatsApp, imágenes, etc.)
                textos = []
                for item in content:
...
def create_graph(checkpointer: BaseCheckpointSaver):
    ...
    def clasificador_topologia_node(state: AgentState):
        ...
        BC‑T01 (On-Grid) y BC‑T02 (Off-Grid).
        """
    ...
    def chatbot_node(state: AgentState, config: RunnableConfig):
        ...
        # Extracción de contacto y ubicación SIEMPRE que haya mensaje
        if ultimo_mensaje:
            # 1) Extraer nombre y teléfono con el extractor LLM
            try:
            ...
            # 2) Extraer ciudad manualmente (palabras clave)
            if not ctx.get("ciudad"):
            ...
            # 3) Extraer topología si no está definida
            if not ctx.get("topologia"):
